...
require_root(){
	["$(id -u)" -eq 0] || die "!!Missing privelages!!";
}
...
detect_backend(){
	if["$SNAP_BACKEND" = "auto"]; then
		if cmd_exists btrfs && awk '$2 == "/"{print $3}'/proc/mounts | grep -p '^btrfs$'; then
			echo "btrfs"; return
		fi
...
rsync_snapshot(){
	...
	for p in $INCLUDE_PATHS; do
		...
		if[-n "latest"] && [-d "$SNAP_ROOT/$latest/$rel"]; then
			linkotp = "--link-dest = $SNAP_ROOT/$latest/$rel"
		fi

		log "rsync snapshot of $p -> $tgt"
		rsync -aHAX --delete --numerical-ids $linkopt \
			$(for e in $EXCLUDE_PATHS; do echo "--exclude = $e"; done) \
			"$p/" "$tgt/" || dir "rsync failed for $p"
	done
	log "rsync snapshot conpleted: $sid"
}
...
create_snapshot(){
	...
	local backend; backend = "$(detect_bakend)"
	...
	case "backend" in
		...
		*) die "Unsupported file system: $backend" ;;
	esac
...
list_snapshot(){
	ls -l "SNAP_ROOT" 2>/dev/null | sort || true;
}
...
Commands:
	...
	restore <id>	Restore snapshot of specified <id>
	list 			List Snapshots
	help			Show this help
EOF
}
